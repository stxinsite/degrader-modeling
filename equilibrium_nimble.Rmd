...
code <- nimbleCode({
  ...
  alpha[N_constructs + 1] <- WT_alpha  # set wild type cooperativity
...
# inits list provided to avoid warnings, but inits_f() will be used for sampling
Rmodel <- nimbleModel(
  code = code,
  constants = constants,
  data = data,
...
# saveRDS(samples_n3, file = "MCMC/nimble_fit_full_fix_WT_alpha.rds")
...
samples_all <- do.call(rbind, samples_n3)  # combine samples from all chains
