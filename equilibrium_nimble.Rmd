...
ggplot(nanobret_96, aes(x = uM, y = mBU_corrected, color = as.factor(construct_int))) +
  ...
  scale_color_hue(labels = c("L1415s", "E1420S", "R69Q", "Y112F", "WT"))
...
# B_x: extracellular degrader concentration
# construct_int: a vector containing integer index corresponding to alpha[i]
# K_AB, K_BC: binary constants of dissociation
# WT_alpha: cooperativity of wild type species
constants <- list(
  N = nrow(nanobret_96),
...
# y: ternary complex concentration multiplied by scaling factor
data <- list(
...
samples_n3 <- readRDS(file = "MCMC/nimble_fit_full_fix_WT_alpha.rds")
```
...
ggs.nanobret %>%
  filter(Parameter %in% c("alpha[L1415S]", "alpha[E1420S]", "alpha[R69Q]", "alpha[Y112F]")) %>%
  ggs_running(greek = TRUE) + 
  theme_bw()

ggs.nanobret %>%
  filter(Parameter %in% c("alpha[L1415S]", "alpha[E1420S]", "alpha[R69Q]", "alpha[Y112F]")) %>%
  ggs_caterpillar(greek = TRUE) + 
  theme_bw() +
