...
ggs.fix %>%
  ...
  theme(# text=element_text(size=20), #change font size of all text
      ...
      legend.text=element_text(size=15), #change font size of legend text
      legend.title=element_text(size=18)) #change font size of legend title 
ggsave("caterpillar_plot_fixed_WT_alpha.png", path = "plots/", width = 12, height = 8, units = "in")
...
B_tArray <- 10 ^ seq(log10(0.00016), log10(5), length.out = 50)
constructArray <- 1:5
constructNames <- c(
  "VHL_WT SMARCA2_L1415S", 
  "VHL_WT SMARCA2_E1420S", 
...
nanobret_96_fit <- data.frame(
  ...
  mBU_corrected = numeric(N_B_t * N_construct), 
...
nanobret_96_fit$source <- "predicted"
nanobret_96_fit$uM <- as.numeric(nanobret_96_fit$uM)
nanobret_96_fit$mBU_corrected <- as.numeric(nanobret_96_fit$mBU_corrected)
```
...
nanobret_96_combined <- rbind(nanobret_96_obs, nanobret_96_fit)
```

## Plot fitted and observed mBU
```{r}
ggplot(nanobret_96_combined, 
       aes(x = uM, 
           y = mBU_corrected, 
          color = factor(Construct, 
                         levels = c("VHL_WT SMARCA2_WT",
  ...
  scale_x_log10() +
  guides(color = guide_legend(title = "Constructs")) +
  labs(title = "Nano-BRET assay measurements and fit",
       x = bquote("[ACBI1] (" * mu * "M)"),
       y = "mBU") +
  theme(# text=element_text(size=20), #change font size of all text
        axis.text=element_text(size=18), #change font size of axis text
        ...
        legend.text=element_text(size=15), #change font size of legend text
        legend.title=element_text(size=18)) #change font size of legend title 
ggsave("mBU_VHL_SMARCA2_fixed_WT_alpha_fit.png", path = "plots/", width = 12, height = 8, units = "in")
```
