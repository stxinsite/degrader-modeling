...
transformed data {
  ...
  real<lower=0> beta = 1;
  real<lower=0> alpha = 30;
}

parameters {
  real<lower=0> C_t;  // total intracellular E3 concentration
  real<lower=0.15*C_t, upper=0.55*C_t> A_t;  // total intracellular target protein concentration
...
model {
  ...
  for (i in 1:N) {
    ...
    theta = [A_t, B_t, C_t, alpha]';  // arguments for cooperative system root-finding
